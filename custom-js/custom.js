  $(document).ready(function(){
    ...
    if (localStorage.getItem('userdetails') === null) {  
        ...
        var productDetails = {p1name:"Blonde Hoodie", p1price:"100"};
    ...
    if (localStorage.getItem('productdetails') === null) {  
        // if productdetails is null, that means it has not been loaded before. we not initialise productdetails object
        var productDetails = {p1name:"Blonde Hoodie", p1price:"100"};

        // now we store the productdetails object as a localstorage object but localstore only stores text and productdetails is a javascript object
        // we convert a javascript object ot a string using JSON.stringify - we are being expedient!
    ...
    // we only run this code if an id of pdetails is on the html page we are currently on - makes the code a little bit more efficient
    // if the length > 0 it means we are on the right page - and we can populdate the form fields!!!
    if ($('#pdetails').length > 0) {
        $('input[name="p1name"]').val(productDetails.p1name);         
        $('input[name="p1price"]').val(userDetails.p1price);

        
        //productDetails.p1name=$('[name="p1name"]').val();
        //productDetails.p1price=$('[name="p1price"]').val();  
    }
    ...
    $('form[name="userdetails"]' ).submit(function( event ) {
        ...
        userDetails.password=$('input[name="password"]').val();
    ...
    $('form[name="productdetails"]' ).submit(function( event ) {
        // if the user updates the user details - we update the userDetails javascript object
        productDetails.p1name=$('[name="p1name"]').val();
        productDetails.p1price=$('[name="p1price"]').val();  
        // finally we convert the javascript object to a string with JSON.stringify and save it to localstorage
